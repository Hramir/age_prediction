...
def load_data(args, datapath):
    if args.dataset == 'cam_can_multiple' or args.dataset == 'binary_cyclic_tree_multiple' or args.dataset == 'meg':
        ...
        logging.info(f"Normalize ADJ : {args.normalize_adj}")
        logging.info(f"Normalize FEATS : {args.normalize_feats}")
...
def load_data_lp(dataset : str, use_feats : bool, data_path : str, use_super_node : bool =False, use_thicks_myelins : bool =True, threshold : float =1000.0):
    if dataset in ['cora', 'pubmed']:
        ...
        import logging
        logging.info(f"Partial Thresholding using : {THRESHOLD_NEW}")
...
def get_adjacency_matrix_for_binary_cyclic_tree(depth):
    ...
    import logging
    logging.info(f"Binary Tree Test: With some probability p = {remove_probability}, Remove edges to nodes")
    ...
    for i in range(len(adj_matrix)):
        ...
        for j in range(len(adj_matrix)):
            ...
            #         adj_matrix[j][i] = 1
            if adj_matrix[i][j] == 1:
                if np.random.uniform() < remove_probability:
                    adj_matrix[i][j] = 0
                    adj_matrix[j][i] = 0
    logging.info("ADJACENCY MATRIX AFTER REMOVING: \n {}".format(adj_matrix))
    return adj_matrix
...
def get_dataset_split_indices(num_sbjs) -> List[List[int]]:
    ...
    # train_split_indices = [i for i in range(num_sbjs)]
    # np.random.shuffle(train_split_indices)

    # logging.info(f"These are the training indices: {train_split_indices}")
    # logging.info(f"These are the val indices: {val_split_indices}")
    # logging.info(f"These are the test indices: {test_split_indices}")

    return train_split_indices, val_split_indices, test_split_indices


def get_adjacency_matrix_cam_can(threshold : float, graph_index : int, data_path : str, use_super_node : bool = False, plv_tensor = None) -> List[List[int]]:
    """
    Create adjacency matrix from threshold number by binarizing the float PLV matrix
    into a 1 and 0 matrix 

    """
    if plv_tensor is None:
        plv_tensor = np.load(os.path.join(data_path, "plv_tensor_592_sbj_filtered.npy"))
        plv_tensor = remove_self_loops(plv_tensor)
    ...
    plv_matrix [plv_matrix < threshold] = 0
    # plv_matrix [plv_matrix >= threshold] = 1
    logging.info(f"PLV Matrix with Partial Thresholding : {plv_matrix}")
...
def get_adjacency_matrices_cam_can(threshold : float, graph_indices : List[int], data_path : str, use_super_node : bool = False, plv_tensor = None) -> List[List[int]]:
    """
    Create adjacency matrix from threshold number by binarizing the float PLV matrix
    into a 1 and 0 matrix 
    ...
    if plv_tensor is None:
        plv_tensor = np.load(os.path.join(data_path, "plv_tensor_592_sbj_filtered.npy"))
        plv_tensor = remove_self_loops(plv_tensor)
    return [get_adjacency_matrix_cam_can(threshold, graph_index, data_path, use_super_node, plv_tensor) for graph_index in graph_indices]
...
def get_adj_mat_dataset_splits_with_features_and_age_labels_and_indices(threshold : float, num_sbjs : int, data_path : str, use_thicks_myelins : bool, use_super_node : bool):
    ...
    # NOTE: Load and pre-process PLV tensor and Thicks Myelins Tensor once to avoid repeatedly loading and processing
    plv_tensor = np.load(os.path.join(data_path, "plv_tensor_592_sbj_filtered.npy"))
    plv_tensor = remove_self_loops(plv_tensor)
    
    thicks_myelins_tensor = np.load(os.path.join(data_path, "cam_can_thicks_myelins_tensor_592_filtered.npy")) # 592 as well  
    
    train_split_adj_matrices = get_adjacency_matrices_cam_can(threshold, train_split_indices, data_path, use_super_node, plv_tensor)
    val_split_adj_matrices = get_adjacency_matrices_cam_can(threshold, val_split_indices, data_path, use_super_node, plv_tensor)
    test_split_adj_matrices = get_adjacency_matrices_cam_can(threshold, test_split_indices, data_path, use_super_node, plv_tensor)
    
    train_feats = get_feature_matrices(train_split_indices, data_path, use_thicks_myelins, use_super_node, plv_tensor, thicks_myelins_tensor)
    val_feats = get_feature_matrices(val_split_indices, data_path, use_thicks_myelins, use_super_node, plv_tensor, thicks_myelins_tensor)
    test_feats = get_feature_matrices(test_split_indices, data_path, use_thicks_myelins, use_super_node, plv_tensor, thicks_myelins_tensor)
...
def get_feature_matrix(graph_index : int, data_path : str, use_thicks_myelins : bool = True, use_super_node : bool = False, plv_tensor = None, thicks_myelins_tensor = None):
    """
    ...
    if plv_tensor is None:
        plv_tensor = np.load(os.path.join(data_path, "plv_tensor_592_sbj_filtered.npy"))
        plv_tensor = remove_self_loops(plv_tensor)
    if thicks_myelins_tensor is None:
        thicks_myelins_tensor = np.load(os.path.join(data_path, "cam_can_thicks_myelins_tensor_592_filtered.npy")) # 592 as well  
    # Thicks Myelins is 2 x 592 x 360
    ...
    if use_only_thicks:
        ...
        logging.info("Using Identity Matrix as Feature Matrix")
        feature_matrix = np.eye(NUM_ROIS)
        
    feature_matrix = np.diag([ct[0] for ct in thick_features])
    logging.info(f"Using CT Diagonal Matrix")
...
def get_feature_matrices(graph_indices : List[int], data_path : str, use_thicks_myelins : bool = True, use_super_node : bool = False, plv_tensor = None, thicks_myelins_tensor = None):
    """
    Feature Matrices for specific subject if 360 x 362 since PLV_vectors are 360
    [CT_0   Myelin_0      PLV_vector_0.T      ]
    [CT_1   Myelin_1      PLV_vector_1.T      ]
    ...
    [CT_359 Myelin_359    PLV_vector_359.T    ]
    ...
    if thicks_myelins_tensor is None:
        thicks_myelins_tensor = np.load(os.path.join(data_path, "cam_can_thicks_myelins_tensor_592_filtered.npy")) # 592 as well  
    return [get_feature_matrix(graph_index, data_path, use_thicks_myelins, use_super_node, plv_tensor, thicks_myelins_tensor) for graph_index in graph_indices]
